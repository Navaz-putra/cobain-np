...
export const generateAuditReport = async (auditId: string) => {
  try {
    ...
    const auditResults: AuditResult[] = answers.map((answer: any) => ({
    ...
    pdf.text("COBIT 2019 Audit Report", 105, 20, { align: "center" });
    ...
    pdf.setFontSize(14);
    pdf.setTextColor(30, 30, 30);
    pdf.text("Maturity Assessment Results", 105, 95, { align: "center" });
    ...
    pdf.setFontSize(14);
    pdf.setTextColor(30, 30, 30);
    pdf.text("Gap Analysis", 105, 20, { align: "center" });
    ...
    pdf.setFontSize(14);
    pdf.setTextColor(30, 30, 30);
    pdf.text("Detailed Assessment Results", 105, 20, { align: "center" });
    ...
    pdf.setFontSize(14);
    pdf.setTextColor(30, 30, 30);
    pdf.text("Recommendations", 105, 20, { align: "center" });
    ...
    pdf.setFontSize(14);
    pdf.setTextColor(30, 30, 30);
    pdf.text("Prioritization Heat Map", 105, 20, { align: "center" });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      pdf.text(`COBIT 2019 Assessment Report - Generated on ${new Date().toLocaleDateString()}`, 105, 295, { align: "center" });
    }
...
const calculateDomainMaturityLevels = (results: any[]) => {
  ...
  results.forEach((result) => {
    if (!domainMap[result.domain_id]) {
      domainMap[result.domain_id] = { total: 0, count: 0 };
    }
...
const addDetailedResultsTable = (pdf: jsPDF, results: any[], startY: number) => {
  ...
  Object.entries(groupedResults).forEach(([domainId, subdomains]) => {
    ...
    Object.entries(subdomains).forEach(([subdomainId, questions]) => {
      ...
      autoTable(pdf, {
        startY: yPos,
        head: [["Question", "Maturity Level", "Notes"]],
        body: tableData,
...
const addRecommendationsTable = (pdf: jsPDF, recommendations: any[], startY: number) => {
  ...
  autoTable(pdf, {
    startY: startY,
    head: [["Domain", "Recommendation", "Priority", "Expected Impact"]],
    body: tableData,
    headStyles: { fillColor: [80, 80, 80] },
    columnStyles: {
      0: { cellWidth: 20 },
